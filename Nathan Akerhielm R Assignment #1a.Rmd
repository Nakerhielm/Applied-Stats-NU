---
title: 'Nathan Akerhielm R Assignment #1a (50 points)'
output:
...
```{r test1a}
sum(7/42,3,35/42)
vec<-c(0,1,2,3,4,13,rep(c(2, -5.1, -23),3), sum(c(7/42,3,35/42)))
print(vec)
```
...
extract<-c(addition[1], addition[length(addition)])
print(extract)
non_extract<-c(addition[2:15])
print(non_extract)
```
...
```{r test1d}
final<-c(extract[1],non_extract, extract[2])
print(final)
sum(final)
...
y<-c(trig(x))
max_x<-round(x[which(trig(x)==max(trig(x)))],3)
max_y<-round(max(y),3)
cat('x=', max_x, 'y=', max_y)
...
plot(x,y,
     col=3,
     main="Trigonometric Function in Color", 
     xlim=c(-2,2),
     ylim=c(0,2))
points(x=max_x, y=max_y, pch=16)
text(max_x, 1.6, labels = "x=1.571, y=1.414")
```
...
x<-seq(from=-2, to=2, by=.001)
inter_x<-x[which(f1(x)==f2(x))]

plot(x, f1(x),
     main="Intersection of Two Curves",
     ylab='y',
     ylim=c(-2,2),
     col=3)

lines(x, f2(x), col=2, lwd=7)
points(inter_x, f1(inter_x), pch=16)
text(0, 1/2, "(0,0)")
...
str(trees)
apply(trees, MARGIN=2,median)
girth<-trees$Girth
#Returns the rows with median girth 
trees[girth== median(girth),]
...
plot(r,area,
     main="Area of Tree vs Radius of Tree",
     xlab="Radius",
     ylab="Area",
     col=11, 
     pch=16)
...
#No extreme outliers b/c max and min values fall within Q1-3*IQR and Q3+3*IQR
trees[area==max(area),]
```
...
object<-cbind(x,y)
f<-apply(object,MARGIN = 2, FUN=IQR)
round(f,4)
...
boxplot(x, horizontal = TRUE,
        col="blue", 
        main="Boxplot of Normal Distribution")

hist(y, col="skyblue",
     main="Histogram of Exponential Distribution")
boxplot(y, horizontal = TRUE,
        col="skyblue", 
        main="Boxplot of Exponential Distribution")
